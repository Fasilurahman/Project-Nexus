...
import { projectSchema, projectUpdateSchema } from "../schemas/projectSchema";
...
const TeamLeadProjectPage: React.FC = () => {
  ...
  const handleSelectEmployee = (employee: Employee) => {
    setSelectedEmployees((prev) => {
      ...
      setNewProject((prevProject) => ({
        ...prevProject,
        teamMembers: [...(prevProject.teamMembers || []), employee], // 🔥 Preserve existing team members
      }));
  ...
  const handleEditProject = (e: React.MouseEvent, project: Project) => {
    ...
    // Populate newProject with the existing project data
    setNewProject({
    ...
    setSelectedEmployees(
      project.teamMembersDetails?.map((member) => member._id) || []
    );
  };
                ...
                >
                  ...
                  <div>
                    ...
                    <FileUpload
                      ...
                      preview={`${S3_PATH}/${newProject.imagePreview}`|| ""}
                ...
                >
                  ...
                  <div>
                    ...
                    <FileUpload
                      onFileSelect={handleImageSelect}
                      label="Drag & drop project image or click to select"
                      preview={newProject.imagePreview ? `${S3_PATH}/${newProject.imagePreview}` : undefined}
                      onRemove={handleRemoveImage}
                    />
                  </div>
                  <div>
                    ...
                    <DocumentUpload
                      onFilesSelect={handleDocumentsSelect}
                      files={newProject.documents.map((doc) => {
                        if (typeof doc === "string") {
                          // Extract the filename from the S3 URL and decode special characters
                          const fileName = decodeURIComponent(
                            doc.split("/").pop() || "Unknown Document"
                          );

                          return {
                            name: fileName.replace(/^\d+-_/, ""), // Remove leading timestamp if present
                            path: `${S3_PATH}/${doc}`,
                            relativePath: `${S3_PATH}/${doc}`,
                            size: 0, // Default size as it's unknown
                          };
