...
const AllTasks = () => {
  ...
  const formatDate = (dateString: string) => {
    try {
      // Try creating the date object from the string
  ...
  const handleCreateTask = async () => {
    try {
      ...
      // Validate new task data using Zod
      taskCreateSchema.parse(newTask);

      console.log("Creating new task:", newTask);
      console.log(teamLeadId, "team lead");
      ...
      // Ensure `assignedTo` exists before accessing `_id`
      const assignedToId = responseData.assignedTo?._id || null;

      // Format the new task data for frontend state
      const newTaskFormatted = {
        ...
        assignedUser: responseData.assignedTo, // Full user object if needed
      };
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      ...
      const responseData = await updateTask(taskId, updatedTaskData);
  ...
  const fetchComments = async (taskId: string) => {
    ...
    const handleAddComment = async () => {
      ...
      const result = await addComment(selectedTask.id, newComment, userId);
    ...
    useEffect(() => {
      const handleNewComment = (comment: Comment) => {
        ...
        if (comment.taskId === selectedTask?.id) {
